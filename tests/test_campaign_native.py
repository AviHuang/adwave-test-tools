"""
P1 Functional Test: Create Native Campaign
"""
import pytest
...
@pytest.mark.asyncio
async def test_create_native_campaign(browser_agent, config):
    """Test creating a Native ad campaign."""
    timestamp = datetime.now().strftime("%H%M%S_%Y%m%d")
    campaign_name = f"{timestamp}_Native"
    ...
    if not campaign_found:
        ...
        assert False, f"Native campaign '{campaign_name}' not found. Extracted campaigns: {campaigns}. Full result: {result[:2000]}"
