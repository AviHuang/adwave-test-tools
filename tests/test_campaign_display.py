"""
P1 Functional Test: Create Display Campaign
"""
import pytest
...
@pytest.mark.asyncio
async def test_create_display_campaign(browser_agent, config):
    """Test creating a Display ad campaign."""
    timestamp = datetime.now().strftime("%H%M%S_%Y%m%d")
    campaign_name = f"{timestamp}_Display"
    ...
    if not campaign_found:
        ...
        assert False, f"Display campaign '{campaign_name}' not found. Extracted campaigns: {campaigns}. Full result: {result[:2000]}"
